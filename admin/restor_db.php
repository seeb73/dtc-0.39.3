...
$mysql_connection = mysqli_connect("$pro_mysql_host", "$pro_mysql_login", "$pro_mysql_pass", "$pro_mysql_db")or die ("Cannot connect to $pro_mysql_host, with db: $pro_mysql_db");

function mysql_table_exists($table){
	$exists = mysqli_query($mysql_connection,"SELECT 1 FROM $table LIMIT 0");
...
function findFieldInTable($table,$field){
	...
	$res = mysqli_query($mysql_connection,$q) or die("Could not query $q!");;
...
for($i=0;$i<$nbr_tables;$i++){
	...
	if( !mysql_table_exists($curtbl) ){
		...
		// echo $q;
		$r = mysqli_query($mysql_connection,$qc)or die("Cannot execute query: \"$qc\" line ".__LINE__." in file ".__FILE__.", mysql said: ".mysql_error());
		...
		for($j=0;$j<$numvars;$j++){
			...
			if($n == 0){
				...
				if( strstr($vc, "auto_increment") != FALSE){
					...
					$r = mysqli_query($mysql_connection,$q)or print("\nCannot execute query: \"$q\" line ".__LINE__." in file ".__FILE__.", mysql said: ".mysql_error()."\n");
				}else{
			...
			// if types don't match, then we issue an ALTER TABLE
				...
				case "text":
					...
					$q2 = "SELECT character_set_name FROM information_schema.`COLUMNS` WHERE table_name = '".$curtbl."' AND column_name = '".$v."'";
                        		$r2 = mysqli_query($mysql_connection,$q2)or die("Cannot execute query: \"$q2\" line ".__LINE__." in file ".__FILE__.", mysql said: ".mysql_error());
...
for($i=0;$i<$n;$i++){
	...
	$r3 = mysqli_query($mysql_connection,$q3)or die("Cannot query ".$q3." line ".__LINE__." file ".__FILE__." sql said ".mysql_error());
