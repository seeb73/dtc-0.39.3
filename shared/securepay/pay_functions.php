...
function validatePaiement($pay_id,$amount_paid,$paiement_type,$secpay_site="none",$secpay_custom_id="0",$total_payed=-1){
	...
	if($amount_paid < $payable_amt)logPay("Amount paid on gateway lower than refund ammount file ".__FILE__." line ".__LINE__);
	if($total_payed != -1){
...
function get_secpay_conf(){
	...
	$fields = mysqli_query($mysqli_connection,"SELECT column_name FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name='$pro_mysql_secpayconf_table' ORDER BY ordinal_position");
