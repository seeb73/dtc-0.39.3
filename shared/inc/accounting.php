...
function mysql_table_exists($dbname,$tableName){
	...
	$tables = array();
	$tablesResult = mysqli_query($mysql_connection,"SHOW TABLES FROM " . $dbname);
...
function sum_http($webname){
	...
	realtime using mod_log_sql2 + patch
			...
			$r_bytes = mysqli_query($mysql_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" !!! ".mysql_error());
			...
			$r_visits = mysqli_query($mysql_connection,$q_visits) or die("Cannot execute query \"$q_visits\" !!! ".mysql_error());
...
//			$visits = 0;
			...
			$r_hosts = mysqli_query($mysql_connection,$q_hosts) or die("Cannot execute query \"$q_hosts\" !!! ".mysql_error());
...
//			$hosts = 0;
			...
			$r_imp = mysqli_query($mysql_connection,$q) or die("Cannot execute query \"$q\" !!! ".mysql_error());
...
//			$imp = 0;
			...
			if(mysqli_num_rows(mysqli_query($mysql_connection,$query_dub))==1){
				...
				mysqli_query($mysql_connection,$query_insert_bytes)or die("Cannot execute query \"$query_insert_bytes\"".mysql_error());
			}else{
				mysql_select_db($conf_mysql_db);
...
function dump_access_log($vhost,$domain,$db_select_name,$current_month,$current_year){
	...
	global $mysql_connection_apache;

	if ($mysql_connection_apache == NULL || $mysql_connection_apache.ping() == false)
	{
		$mysql_connection_apache = mysqli_connect("$conf_mysql_host", "$conf_mysql_login", "$conf_mysql_pass", "apachelogs")or die ("Cannot connect to $conf_mysql_host, with db: apachelogs");
	}
	$query = "SELECT MAX(time_stamp) AS end FROM `".$db_select_name."`";
	$result = mysqli_query($mysql_connection_apache,$query) or die("Cannot execute query \"$query\" line ".__LINE__." file ".__FILE__.": ".mysql_error());
	...
	if($start!=0 && $end!=0){
		...
		for($year=$year_start;$year<=$year_end;$year++){
			for($month=$start_month;$month<=12;$month++){
				...
				$result_admin = mysqli_query($mysql_connection,$query_admin) or die("Cannot execute query \"$query_admin\" line ".__LINE__." file ".__FILE__.": ".mysql_error());
...
' "'.$rezar["referer"].'" "'.$rezar["agent"].'"'."\n";
					...
					mysql_select_db("apachelogs");
					mysqli_query($mysql_connection,$query_del) or die("Cannot execute query \"$query_del\" line ".__LINE__." file ".__FILE__.": ".mysql_error());
...
function check_sum($db_select_name,$selected_month_start,$selected_month_end,$webname,$subdomain_name){
	...
	// Get bytes_sent
	$q_bytes = "SELECT SUM( bytes_sent ) AS amount FROM `".$db_select_name."` WHERE time_stamp>=".$selected_month_start." AND time_stamp<=".$selected_month_end;
	$r_bytes = mysqli_query($mysql_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" line ".__LINE__." file ".__FILE__.": ".mysql_error());
	$bytes_sent = mysql_result($r_bytes,0,"amount");
	$visits = 0;
	...
	$r_imp = mysqli_query($mysql_connection,$q_imp) or die("Cannot execute query \"$q_imp\" !!! ".mysql_error());
	$imp = mysqli_num_rows($r_imp);
...
//	mysql_select_db("dtc");
	...
	$result_l = mysqli_query($mysql_connection,$q) or die("Cannot execute query \"$q\"");
	...
	mysqli_query($mysql_connection,$q2) or die("Cannot execute query \"$q2\" !".mysql_error());
}
