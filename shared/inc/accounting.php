...
function mysqli_table_exists($dbname,$tableName){
	global $mysqli_connection;
	$tables = array();
	$tablesResult = mysqli_query($mysqli_connection,"SHOW TABLES FROM " . $dbname);
...
function fetchHTTPInfo($webname){
	...
	$amount = mysqli_result($result,0,"transfer");
...
function fetchFTPInfo($webname){
	...
	$total_ftp_amount = mysqli_result($result,0,"transfer");
...
function fetchSMTPInfo($webname){
	...
	mysqli_error($mysqli_connection)." line ".__LINE__." file ".__FILE__);
	if(mysqli_num_rows($r) < 1)
		$total_smtp_amount = 0;
	else
		$total_smtp_amount = mysqli_result($r,0,"transfer");
...
function fetchIMAPInfo($webname){
	...
	if(mysqli_num_rows($r) < 1)
		...
		$total_imap_amount = mysqli_result($r,0,"transfer");
...
function fetchPOPInfo($webname){
	...
	if(mysqli_num_rows($r) < 1)
		...
		$total_pop_amount = mysqli_result($r,0,"transfer");
...
function sum_http($webname){
	...
	for($i=0;$i<$num_rows;$i++){
		$subdomain_name = mysqli_result($result,$i,"subdomain_name");
		$db_webname=strtr($webname,'.','_');
		$db_select_name = $db_webname.'$'.$subdomain_name.'$'."xfer";

		if(mysqli_table_exists("apachelogs",$db_select_name)){
	...
	realtime using mod_log_sql2 + patch
			...
			$r_bytes = mysqli_query($mysqli_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" !!! ".mysqli_error($mysqli_connection));
			$bytes_sent = mysqli_result($r_bytes,0,"amount");
			//$bytes_sent = 0;
			// Get visits
			$q_visits = "SELECT DISTINCT id FROM `".$db_select_name."` WHERE time_stamp>=".$selected_month_start." AND time_stamp<=".$selected_month_end;
			$r_visits = mysqli_query($mysqli_connection,$q_visits) or die("Cannot execute query \"$q_visits\" !!! ".mysqli_error($mysqli_connection));
			$visits = mysqli_num_rows($r_visits);
//			$visits = 0;
			...
			$r_hosts = mysqli_query($mysqli_connection,$q_hosts) or die("Cannot execute query \"$q_hosts\" !!! ".mysqli_error($mysqli_connection));
			$hosts = mysqli_num_rows($r_hosts);
//			$hosts = 0;

			// Get impressions
			$q = "SELECT id FROM `".$db_select_name."` WHERE time_stamp>=".$selected_month_start." AND time_stamp<=".$selected_month_end.";";
			$r_imp = mysqli_query($mysqli_connection,$q) or die("Cannot execute query \"$q\" !!! ".mysqli_error($mysqli_connection));
			$imp = mysqli_num_rows($r_imp);
//			$imp = 0;
			...
			if(mysqli_num_rows(mysqli_query($mysqli_connection,$query_dub))==1){
				...
				mysqli_query($mysqli_connection,$query_insert_bytes)or die("Cannot execute query \"$query_insert_bytes\"".mysqli_error($mysqli_connection));
			}else{
				mysqli_select_db($conf_mysql_db);
...
function dump_access_log($vhost,$domain,$db_select_name,$current_month,$current_year){
	...
	global $mysqli_connection_apache;

	if ($mysqli_connection_apache == NULL || $mysqli_connection_apache.ping() == false)
	{
		$mysqli_connection_apache = mysqli_connect("$conf_mysql_host", "$conf_mysql_login", "$conf_mysql_pass", "apachelogs")or die ("Cannot connect to $conf_mysql_host, with db: apachelogs");
	}
	$query = "SELECT MAX(time_stamp) AS end FROM `".$db_select_name."`";
	$result = mysqli_query($mysqli_connection_apache,$query) or die("Cannot execute query \"$query\" line ".__LINE__." file ".__FILE__.": ".mysqli_error($mysqli_connection));
	$end = mysqli_result($result,0,"end");
	...
	if($start!=0 && $end!=0){
		...
		for($year=$year_start;$year<=$year_end;$year++){
			for($month=$start_month;$month<=12;$month++){
				...
				$result_admin = mysqli_query($mysqli_connection,$query_admin) or die("Cannot execute query \"$query_admin\" line ".__LINE__." file ".__FILE__.": ".mysqli_error($mysqli_connection));
				$adm_login = mysqli_result($result_admin,0,"owner");
...
' "'.$rezar["referer"].'" "'.$rezar["agent"].'"'."\n";
					...
					mysqli_select_db("apachelogs");
					mysqli_query($mysqli_connection,$query_del) or die("Cannot execute query \"$query_del\" line ".__LINE__." file ".__FILE__.": ".mysqli_error($mysqli_connection));
...
function check_sum($db_select_name,$selected_month_start,$selected_month_end,$webname,$subdomain_name){
	...
	// Get bytes_sent
	$q_bytes = "SELECT SUM( bytes_sent ) AS amount FROM `".$db_select_name."` WHERE time_stamp>=".$selected_month_start." AND time_stamp<=".$selected_month_end;
	$r_bytes = mysqli_query($mysqli_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" line ".__LINE__." file ".__FILE__.": ".mysqli_error($mysqli_connection));
	$bytes_sent = mysqli_result($r_bytes,0,"amount");
	$visits = 0;
	...
	$r_imp = mysqli_query($mysqli_connection,$q_imp) or die("Cannot execute query \"$q_imp\" !!! ".mysqli_error($mysqli_connection));
	$imp = mysqli_num_rows($r_imp);
...
//	mysqli_select_db("dtc");
	...
	$result = mysqli_query($mysqli_connection,$query) or die("Cannot execute query \"$query\" !".mysqli_error($mysqli_connection));
	$num_rows = mysqli_num_rows($result);
	$total_ftp_amount = mysqli_result($result,0,"dl_amount") + mysqli_result($result,0,"ul_amount");
	$query = "UPDATE $pro_mysql_ftp_table set ul_bytes=0, dl_bytes=0 WHERE hostname='".$webname."'";
	mysqli_query($mysqli_connection,$query) or die("Cannot execute query \"$query\"".mysqli_error($mysqli_connection));
	...
	$result_l = mysqli_query($mysqli_connection,$q) or die("Cannot execute query \"$q\"");
	$numrow_acctable = mysqli_num_rows($result_l);
	...
	mysqli_query($mysqli_connection,$q2) or die("Cannot execute query \"$q2\" !".mysqli_error($mysqli_connection));
}
