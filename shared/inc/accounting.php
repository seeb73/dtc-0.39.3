...
function fetchSMTPInfo($webname){
	...
	mysqli_error()." line ".__LINE__." file ".__FILE__);
...
function sum_http($webname){
	...
	realtime using mod_log_sql2 + patch
			...
			$r_bytes = mysqli_query($mysqli_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" !!! ".mysqli_error());
			...
			$r_visits = mysqli_query($mysqli_connection,$q_visits) or die("Cannot execute query \"$q_visits\" !!! ".mysqli_error());
...
//			$visits = 0;
			...
			$r_hosts = mysqli_query($mysqli_connection,$q_hosts) or die("Cannot execute query \"$q_hosts\" !!! ".mysqli_error());
...
//			$hosts = 0;
			...
			$r_imp = mysqli_query($mysqli_connection,$q) or die("Cannot execute query \"$q\" !!! ".mysqli_error());
...
//			$imp = 0;
			...
			if(mysqli_num_rows(mysqli_query($mysqli_connection,$query_dub))==1){
				...
				mysqli_query($mysqli_connection,$query_insert_bytes)or die("Cannot execute query \"$query_insert_bytes\"".mysqli_error());
			}else{
				mysqli_select_db($conf_mysql_db);
...
function dump_access_log($vhost,$domain,$db_select_name,$current_month,$current_year){
	...
	$result = mysqli_query($mysqli_connection_apache,$query) or die("Cannot execute query \"$query\" line ".__LINE__." file ".__FILE__.": ".mysqli_error());
	...
	if($start!=0 && $end!=0){
		...
		for($year=$year_start;$year<=$year_end;$year++){
			for($month=$start_month;$month<=12;$month++){
				...
				$result_admin = mysqli_query($mysqli_connection,$query_admin) or die("Cannot execute query \"$query_admin\" line ".__LINE__." file ".__FILE__.": ".mysqli_error());
...
' "'.$rezar["referer"].'" "'.$rezar["agent"].'"'."\n";
					...
					mysqli_select_db("apachelogs");
					mysqli_query($mysqli_connection,$query_del) or die("Cannot execute query \"$query_del\" line ".__LINE__." file ".__FILE__.": ".mysqli_error());
...
function check_sum($db_select_name,$selected_month_start,$selected_month_end,$webname,$subdomain_name){
	...
	// Get bytes_sent
	$q_bytes = "SELECT SUM( bytes_sent ) AS amount FROM `".$db_select_name."` WHERE time_stamp>=".$selected_month_start." AND time_stamp<=".$selected_month_end;
	$r_bytes = mysqli_query($mysqli_connection,$q_bytes) or die("Cannot execute query \"$q_visits\" line ".__LINE__." file ".__FILE__.": ".mysqli_error());
	$bytes_sent = mysqli_result($r_bytes,0,"amount");
	$visits = 0;
	...
	$r_imp = mysqli_query($mysqli_connection,$q_imp) or die("Cannot execute query \"$q_imp\" !!! ".mysqli_error());
	$imp = mysqli_num_rows($r_imp);
...
//	mysqli_select_db("dtc");
	...
	mysqli_query($mysqli_connection,$q2) or die("Cannot execute query \"$q2\" !".mysqli_error());
}
