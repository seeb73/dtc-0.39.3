...
function deleteMysqlUserAndDB($adm_login){
	...
	$r = mysqli_query($mysql_connection,$q)or die("Cannot query $q line ".___LINE__." file".__FILE__);
	$n = mysqli_num_rows($r);
	for($j=0;$j<$n;$j++){
		...
		for($i=0;$i<$num_rows;$i++){
			...
			if($db != $conf_mysql_db && $db != "mysql"){
				...
				mysqli_query($mysql_connection,$query2)or die("Cannot execute query \"$query2\" line ".__line__." file ".__FILE__." mysql said: ".mysql_error());
			}
		}
		// Prevent system user from deletion
		if($mysql_user != "mysql" && $mysql_user != "root"){
			$query = "DELETE FROM db WHERE User='$mysql_user';";
			mysqli_query($mysql_connection,$query)or die("Cannot execute query \"$query\" !!!");
...
function deleteUserDomain($adm_login,$adm_pass,$deluserdomain,$delete_directories = false){
	...
	$result = mysqli_query($mysql_connection,$adm_query)or die("Cannot execute query \"$adm_query\" line ".__LINE__." file ".__FILE__."sql said ".mysql_error());
	$num_rows = mysqli_num_rows($result);
	...
	$r = mysqli_query($mysql_connection,$adm_query)or die("Cannot execute query \"$adm_query\" line ".__LINE__." file ".__FILE__);
	$n = mysqli_num_rows($r);
	if($n == 0){
...
function DTCdeleteAdmin ($adm_to_del) {
	...
	$result = mysqli_query($mysql_connection,$adm_query)or die("Cannot execute query \"$adm_query\" !!!");
	$num_rows = mysqli_num_rows($result);
	if($num_rows != 1)
		...
		if($tik_a != ""){
			...
			for($j=0;$j<$num;$j++){
				...
				if(isRandomNum($id)){
					...
					mysqli_query($mysql_connection,$qd)or die("Cannot execute query \"$qd\" line ".__LINE__." file ".__FILE__." sql said: ".mysql_error());
				}
			}
		}
	}
